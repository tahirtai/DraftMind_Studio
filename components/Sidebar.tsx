...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
    ...
    return (
        <aside className="w-64 h-full bg-surface-dark border-r border-border-dark flex flex-col transition-all duration-300">
            {/* Logo */}
            ...
            <div className="flex flex-col gap-6 p-6 overflow-y-auto">
                ...
                <nav className="flex flex-col gap-1">
                    <NavLink to="/dashboard" onClick={onClose} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors w-full text-left group ${isActive ? 'bg-gray-100 dark:bg-white/5 text-primary dark:text-white' : 'text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                        <span className="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">home</span>
                        <span>Home</span>
                    </NavLink>
                    <NavLink to="/projects" end onClick={onClose} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors w-full text-left group ${isActive ? 'bg-gray-100 dark:bg-white/5 text-primary dark:text-white' : 'text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                        <span className="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">folder_open</span>
                        <span>All Projects</span>
                    </NavLink>
                    <NavLink to="/analytics" onClick={onClose} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors w-full text-left group ${isActive ? 'bg-gray-100 dark:bg-white/5 text-primary dark:text-white' : 'text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                        <span className="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">bar_chart</span>
                        <span>Analytics</span>
                    </NavLink>
                    <NavLink to="/billing" onClick={onClose} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors w-full text-left group ${isActive ? 'bg-gray-100 dark:bg-white/5 text-primary dark:text-white' : 'text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                    ...
                    {favoriteProjects.length > 0 && (
                        <>
                            ...
                            {favoriteProjects.map((p, i) => (
                                <NavLink key={p.id} to={`/projects/${p.id}`} onClick={onClose} className="flex items-center gap-3 px-3 py-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
            ...
            <div className="flex flex-col gap-2 p-4 border-t border-border-dark bg-background-dark/30">
                <NavLink to="/settings" onClick={onClose} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors w-full text-left ${isActive ? 'bg-gray-100 dark:bg-white/5 text-primary dark:text-white' : 'text-text-secondary hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                ...
                {/* User Profile */}
                <div className="flex items-center gap-3 px-3 py-2 mt-2 rounded-xl bg-surface-dark border border-border-dark cursor-pointer hover:bg-gray-100 dark:hover:bg-white/5 transition-colors" onClick={() => { navigate('/profile'); if (onClose) onClose(); }}>
